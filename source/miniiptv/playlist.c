...
static void parse_extinf(const char *line, MiniIptvChannel *pending) {
    ...
    if (comma && comma[1]) copy_bounded(pending->name, sizeof(pending->name), comma + 1, strlen(comma + 1));
    else copy_bounded(pending->name, sizeof(pending->name), "Unnamed channel", 15);
